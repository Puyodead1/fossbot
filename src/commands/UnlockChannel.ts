    ...
    public async execute(msg: Message, args: string[]): Promise<any> {
        ...
        if (!perms) {
            ...
            if (perms.allow.has("SendMessages")) {
                const embed = new EmbedBuilder().setDescription(`🤔 Channel is not locked`).setColor("Red").setTimestamp().setFooter({
                    text: msg.author.tag,
        ...
        const embed = new EmbedBuilder().setDescription(`🔓 Unlocked channel <#${channel.id}>`).setColor("Red").setTimestamp().setFooter({
            text: msg.author.tag,
            iconURL: msg.author.displayAvatarURL(),
        });
        return await msg.channel.send({
